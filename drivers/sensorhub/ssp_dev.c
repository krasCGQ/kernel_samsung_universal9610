...
static int ssp_parse_dt(struct device *dev, struct ssp_data *data)
{
	...
	if (of_property_read_u16_array(np, "ssp-prox-thresh-addval",
	                              data->prox_thresh_addval, PROX_THRESH_SIZE)) {
	...
	if (of_property_read_u32_array(np, "ssp-light-cam-lux",
		...
		data->camera_lux_hysteresis[0] = 0;
