...
static void recovery_mcu(struct ssp_data *data, int reason)
{
...
static void debug_work_func(struct work_struct *work)
{
	...
	ssp_infof("Sensor state: 0x%llx, En: 0x%llx Reset cnt: %u, Comm fail: %u, Dump cnt : %d, Time out: %u No event : %u",
	          data->sensor_probe_state, data->sensor_en_state, data->cnt_reset, data->cnt_com_fail, data->cnt_dump,
	...
	if(data->cnt_dump < 0 && data->cnt_dump >= -10) {
