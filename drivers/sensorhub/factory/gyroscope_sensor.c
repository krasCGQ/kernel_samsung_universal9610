...
void select_gyro_ops(struct ssp_data *data)
{
	...
	if(count > 1) {
		if(get_sensorname(data, SENSOR_TYPE_GYROSCOPE, name, sizeof(name)) != SUCCESS){
			i = 0;
		} else {
			for(i = 0; i < count ; i++)
			{
...
{
#if defined(CONFIG_SENSORS_SSP_GYROSCOPE_ICM42605M)
	data->gyro_ops = get_gyroscope_icm42605m_function_pointer(data);
#elif defined(CONFIG_SENSORS_SSP_GYROSCOPE_LSM6DSL)
	data->gyro_ops = get_gyroscope_lsm6dsl_function_pointer(data);
#elif defined(CONFIG_SENSORS_SSP_GYROSCOPE_K6DS3TR)
	data->gyro_ops = get_gyroscope_k6ds3tr_function_pointer(data);
#endif
