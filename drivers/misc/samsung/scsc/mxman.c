...
int disable_recovery_handling = 1;  /* MEMDUMP_FILE_FOR_RECOVERY : for .memdump.info */
...
{
	static bool disable_recovery_from_memdump_file;
	static bool cached_memdump_file_value;
	if (disable_recovery_handling == MEMDUMP_FILE_FOR_RECOVERY) {
		if (cached_memdump_file_value) {
			SCSC_TAG_INFO(MX_FILE, "Auto-recovery: %s\n",
				disable_recovery_from_memdump_file ? "off" : "on");
...
#define MX140_MEMDUMP_INFO_FILE	"/data/misc/conn/.memdump.info"
			...
			r = mx140_request_file(NULL, MX140_MEMDUMP_INFO_FILE, &firm);
			if (r) {
				SCSC_TAG_WARNING(MX_FILE, "Error Loading %s file %d\n", MX140_MEMDUMP_INFO_FILE, r);
			...
			SCSC_TAG_INFO(MX_FILE, "%s:%c Auto-recovery: %s\n",
					MX140_MEMDUMP_INFO_FILE, *firm->data,
					disable_recovery_from_memdump_file ? "off" : "on");
			mx140_release_file(NULL, firm);

			cached_memdump_file_value = true;
